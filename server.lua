...
AddEventHandler('chatMessage', function(source, color, msg, args)
	cm = stringsplit(msg, " ")
	...
	if cm[1] == "/support" then
		...
		if tablelength(cm) > 1 then
		    ...
		    TriggerClientEvent("sendSupport", -1, source, names1, textmsg)
...
AddEventHandler('checkadmin', function(n1, tmsg, ii)
	local id = source
	if isAdmin(id) then
		TriggerClientEvent("sendSupportToAllAdmins", -1, source, n1, tostring(tmsg), ii)
	else
	end
end)
